...
# 초기화
pygame.mixer.init()
pygame.mixer.music.load('short_alarm.mp3')
...
# 비디오 스트림의 프레임 반복
def tts_and_chatgpt(message):
    global ALARM_ON
    pygame.mixer.music.play()
    cv2.putText(gray, "Drowsy", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 2)
...
while True:
    ...
    for rect in rects:
        ...
        if len(PERCLOS_FRAMES) >= PERCLOS_INTERVAL * 120:  # 30fps 가정
            perclos_value = sum(1 for e in PERCLOS_FRAMES if e < PERCLOS_THRESHOLD) / len(PERCLOS_FRAMES)

            # PERCLOS 값이 임계값보다 크면 알람 울림
            if perclos_value >= PERCLOS_THRESHOLD and not ALARM_ON:
                ALARM_ON = True
                # TTS 및 ChatGPT 소통을 쓰레드로 시작
                cv2.putText(gray, "Drowsy", (10, 230), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 2)
                cv2.imshow("drowsiness_detection", gray)
                cv2.waitKey(1500)  # 0.5초 동안 대기

                Thread(target=tts_and_chatgpt, args=("졸음 감지",)).start()

                PERCLOS_FRAMES = []  # PERCLOS_FRAMES 초기화
                last_perclos_time = time.time()  # 타이머 초기화

        if time.time() - last_perclos_time >= PERCLOS_INTERVAL:
    ...
    # 프레임에 FPS 텍스트 추가
    cv2.putText(gray, fps_text, (180, 230), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
